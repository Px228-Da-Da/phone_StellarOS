    ...
    while (!m.stopped) {
        ...
        case OP_APPEND: {
            ...
            if (n >= cap) {
                ...
                /*
                 * Если список лежит последним — растим его на месте.
                 *
                 * Памяти под объекты машина не возвращает вовсе: выдача
                 * это сдвиг указателя, освобождения нет. Значит каждое
                 * удвоение списка бросало старый кусок навсегда, и
                 * «добавлять в цикле» — самое обычное, что делают с
                 * списком, — протекало вдвое от итогового размера.
                 *
                 * Ровно на этом и останавливался калькулятор: показ
                 * числа складывает цифры в список, и на длинных числах
                 * куча кончалась.
                 *
                 * Когда кусок последний, копировать и бросать незачем:
                 * достаточно подвинуть край. Для добавления в цикле это
                 * убирает потерю целиком.
                 */
                if ((vm_u32)data + (vm_u32)cap == m.hp &&
                    m.hp + (vm_u32)cap <= VM_HEAP) {
                    m.hp += (vm_u32)cap;
                    m.heap[h + 2] = grown;
                } else {
