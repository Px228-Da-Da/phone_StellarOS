 ...
 *
 * Метод — обычная функция, у которой первый аргумент self. Имя ей
...
/* Первый аргумент метода. Слово одно и пишется по-английски, как и все
 * остальные служебные слова языка: class, def, if, while, return. */
static int is_self_name(const char *n)
{
    return !strcmp(n, "self");
}
 ...
 * его первый аргумент — self. Отдельного кода для этого не нужно: имя
 * ему уже дано составное при предпросмотре, а self становится обычной
