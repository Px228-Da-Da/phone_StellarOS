...
static void restart_with(const char *path)
{
     ...
     */
    /*
     * Корень исходников передаём себе же обязательно.
     *
     * Без него после первого сохранения среда теряла, где лежат
     * приложения, и начинала искать их в текущей папке: список слева
     * пустел, а компилятор жаловался на «./calc/main.ht». Всё, что нужно
     * новому образу, должно уехать в аргументах — от прежнего не
     * остаётся ничего, кроме открытых файлов.
     */
    if (ide_mode)
        execl(self_path, self_path, path, port_arg, "--ide", "--снова",
              "--корень", apps_dir, (char *)NULL);
    else
        execl(self_path, self_path, path, port_arg, "--снова",
              "--корень", apps_dir, (char *)NULL);
