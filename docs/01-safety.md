...
## Правило №4 — не раздавать чужую память

Снято с устройства (`device-info/dt-tree.txt`): в `/reserved-memory` двадцать
один блок. Там живут ATF, TEE, доверенная память, буферы модема, фреймбуфер,
pstore. Всё это лежит **внутри** той же DRAM, которой распоряжается наш
аллокатор страниц, и разбросано по ней кусками.

Обычная ошибка чтения тут не грозит — грозит хуже. Защищённые области закрыты
аппаратно, контроллером EMI: обращение к ним из нашего EL1 не вернёт мусор,
а вызовет нарушение защиты и немедленную перезагрузку. Отладить такое
на телефоне почти невозможно — просто ресет без единого сообщения.

Разбор `/reserved-memory` уже сделан: ядро читает дерево, которое LK передаёт
ему в x0, и помечает все такие блоки занятыми. На сборке для телефона это
условие жёсткое — **если список защищённых областей пуст, pmm не запускается
вовсе**, и ядро остаётся без кучи и без задач. Молчаливого «а вдруг обойдётся»
здесь нет.

Что проверить глазами на первой же прошивке, до всего остального:

```
ЗАЩИЩЕНО : 21 ОБЛАСТЕЙ ИЗ DTB
```

Двадцать один — столько блоков в `/reserved-memory` этого устройства
(`device-info/dt-tree.txt`). Цифра меньше означает, что часть областей ядро
не увидело, и раздаст их под кучу. В этом случае — сразу откат, разбираться
на живом телефоне нельзя.

## Правило №5 — сначала QEMU

Каждая новая фича сначала работает в `make run`. На телефон льём
