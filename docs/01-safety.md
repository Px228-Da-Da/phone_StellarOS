...
Поэтому: **не прошивать своё ядро, пока pmm не научился читать
`/reserved-memory` из DTB** и помечать эти блоки занятыми. Разбор DTB стоит
в этапе 5, и до него первая прошивка не имеет смысла. Пока же аллокатору
отдаётся заведомо малая часть памяти, а точные адреса взять неоткуда:
свойства `reg` без root не читаются, зато тот же DTB LK передаёт нашему
ядру в x0 — оттуда и возьмём.
