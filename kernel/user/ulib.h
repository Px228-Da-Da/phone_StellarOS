 ...
 * Выключить телефон.
 *
 * В отличие от перезагрузки, ВОЗВРАЩАЕТСЯ при неудаче — с кодом PSCI.
 * Выключением распоряжается прошивка в EL3, и если она этого не умеет,
 * система говорит об этом честно, а не гасит экран, оставив телефон
 * включённым. Ноль отсюда не приходит никогда: при успехе возвращаться
 * уже некуда.
 */
static inline s64 power_off(void)
{
    return sys(SYS_POWEROFF, 0, 0, 0, 0, 0, 0);
}

/*
 * Забрать приложение, присланное по проводу.
 *
 * Возвращает длину или 0, если ничего не присылали — тогда работает то,
 * что вшито в систему. Копия, а не ссылка: буфер приёма живёт в памяти
 * ядра, и показывать на него из EL0 нельзя.
 */
static inline s64 app_image(void *dst, u64 max)
{
    return sys(SYS_APP, (u64)(unsigned long)dst, max, 0, 0, 0, 0);
}
 ...
 */
/* Написать строку. Возвращает её ширину в точках — по ней выстраивают
 * буквы одну за другой, когда шрифт не равноширинный. */
 ...
 *
 * Шрифт пропорциональный, поэтому «сколько букв × ширина буквы» —
...
#define CLOCK_DOW(v)    ((u32)(((u64)(v) >> 48) & 0x07))
...
/*
 * Залить кусок своей половины окна обоями. 0 — получилось, -1 — обоев
 * нет, и тогда рисовать фон программе надо самой.
 *
 * Половину называем тем же числом, каким потом просим её показать.
 * Указатель не передаём: буфер и так наш, а ядру дешевле найти его по
 * задаче, чем проверять десять мегабайт чужого адреса.
 */
static inline s64 wallpaper(u32 half, u32 x, u32 y, u32 w, u32 h)
{
    return sys(SYS_WALL, half, x, y, w, h, 0);
}

/*
 * Показывать ли обои в системных полосах сверху и снизу.
 *
 * На замке — да, иначе картинка режется чёрным поперёк верха. На тёмном
 * рабочем столе — нет, иначе светлая лента выглядит поломкой.
 */
static inline s64 strip_wallpaper(u32 on)
{
    return sys(SYS_STRIP, on, 0, 0, 0, 0, 0);
}

/*
 * Сообщить системной полосе, насколько уехало содержимое.
 *
 * Нужно затем, чтобы значок заряда ехал вместе с экраном блокировки, а
 * не оставался висеть один. Полосу рисует ядро, жест видит программа —
 * значит сказать об этом должна она.
 */
static inline s64 strip_lift(u32 on, u32 px)
{
    return sys(SYS_STRIP, on, px, 0, 0, 0, 0);
}

/*
 * Батарея одним числом. Разбирается макросами ниже.
 *
 * BATT_OK обязателен к проверке: пока задача питания не сделала первый
 * замер, всё остальное здесь нули, и нарисованная по ним батарея
 * показала бы пустую. Пустая батарея на заряженном телефоне — худшая из
 * возможных неправд, потому что выглядит она совершенно правдоподобно.
 */
static inline s64 battery_now(void)
{
    return sys(SYS_BATT, 0, 0, 0, 0, 0, 0);
}

#define BATT_PERCENT(v) ((u32)((u64)(v) & 0xFF))
#define BATT_CHARGING(v)((u32)(((u64)(v) >> 8) & 1))
#define BATT_OK(v)      ((u32)(((u64)(v) >> 9) & 1))
#define BATT_MV(v)      ((u32)(((u64)(v) >> 16) & 0xFFFF))
#define BATT_MA(v)      ((int)(short)(((u64)(v) >> 32) & 0xFFFF))
...
/* Ждать касания сколько угодно. Возвращает 1, когда событие в t. */
...
/*
 * Ждать касания не дольше ms миллисекунд: 1 — дождались, 0 — время вышло.
 *
 * Нужно всему, что движется само: прокрутка по инерции, мигание курсора,
 * часы. Пальца на экране в этот момент нет, а кадры рисовать надо, и
 * бесконечное ожидание события тут означало бы неподвижную картинку.
 */
static inline s64 input_wait(struct touch *t, u32 ms)
{
    return sys(SYS_INPUT, (u64)t, ms, 0, 0, 0, 0);
}
