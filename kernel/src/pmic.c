...
/*
 * Прочитать, НЕ ДОЖИДАЯСЬ шины: занято — сразу отказ.
 *
 * Нужно ровно одному месту — опросу кнопки из обработчика таймера, — и
 * появилось после того, как я поставил туда обычное чтение. Ошибка была
 * не в мелочи, а в устройстве: этот замок берут задача батареи, часы и
 * зарядка, держат столько, сколько им надо, и держать вправе. Тик же
 * обязан вернуться всегда — им доставляется вывод в консоль, по нему
 * считается время и сменяются задачи. Связав одно с другим, я сделал так,
 * что застрявший на шине читатель останавливает систему целиком, и она
 * при этом не может даже сказать, что встала.
 *
 * Отказ здесь ничего не стоит: кнопку опрашивают пятьдесят раз в
 * секунду, и пропустить один опрос незаметно.
 */
static int pmic_read_try(u32 reg, u16 *out)
{
    u64 flags = irq_save();
    int rc;

    if (!spin_trylock(&pmic_lock)) {
        irq_restore(flags);
        return -1;
    }
    rc = pmic_read_locked(reg, out);
...
/*
 * Опрос кнопки на тике, и почему он здесь, а не в оболочке.
 *
 * Раньше кнопку спрашивала оболочка из своего цикла — раз в двести
 * миллисекунд, когда экран погашен. Короткое нажатие длится восемьдесят
 * с чем-то, и оно целиком помещалось между двумя опросами: телефон
 * оставался тёмным, хотя работал. Снаружи это выглядело как «не
 * отвечает», и разницы с зависанием не было никакой.
 *
 * Кнопка — единственный путь обратно с погашенного экрана, и терять на
 * нём события нельзя вовсе. Поэтому опрос переехал сюда: тик приходит
 * сто раз в секунду что бы ни делали задачи, и даже если оболочка занята
 * или сломана, нажатие всё равно будет замечено и запомнено.
 *
 * Берём каждый второй тик — двадцать миллисекунд. Чаще незачем: человек
 * не нажимает быстрее, а каждый опрос это транзакция по шине питания.
 *
 * Запоминаем ФРОНТ, а не состояние: пока палец на кнопке, состояние
 * держится, и по нему одно нажатие сработало бы десятки раз.
 */
static volatile int pk_prev;
static volatile int pk_latched;

/*
 * Здесь НЕТ проверки номера ядра, и это важно.
 *
 * Сначала опрос жил в обработчике таймера, а тик приходит на все восемь
 * ядер — там проверка «работаю только на нулевом» была необходима, иначе
 * мы дёргали бы шину восьмикратно. Перенеся опрос в обычную задачу, я
 * проверку не убрал, и она превратилась из полезной во вредную: задача
 * существует в одном экземпляре, а планировщик гоняет её по ядрам, и на
 * нулевом она оказывалась примерно раз из восьми. Кнопка от этого
 * срабатывала через раз, а чаще не срабатывала вовсе.
 *
 * Частоту задаёт сон вызывающего, а не счётчик здесь: делитель тоже был
 * нужен только тику.
 */
void pmic_powerkey_poll(void)
{
    u16 v;
    int now;

    /*
     * Занятую шину пропускаем молча и НЕ трогаем pk_prev: пропущенный
     * опрос это незнание, а не «кнопка отпущена». Записав сюда ноль, мы
     * выдумали бы отпускание, которого не было, и следующий опрос счёл бы
     * его новым нажатием.
     */
    if (pmic_read_try(MT6358_TOPSTATUS_REG, &v) != 0)
        return;

    now = (v & PWRKEY_MASK) ? 0 : 1;
    if (now && !pk_prev) {
        pk_latched = 1;
        kprintf("КНОПКА   : НАЖАТИЕ\n");
    }
    pk_prev = now;
}

int pmic_powerkey_taken(void)
{
    int v = pk_latched;
...
void pmic_powerkey_poll(void) { }
int  pmic_powerkey_taken(void) { return 0; }
