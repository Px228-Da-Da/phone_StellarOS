...
static void snapshot(void)
{
     ...
     */
    /*
     * Ждём ВТОРОЙ конец кадра, а первый пропускаем.
     *
     * Ожидание готовности убрало недобор — СОСТ стало 00000001 вместо
     * 00000003, — но записанным всё равно вышло около семидесяти
     * процентов, и не хватало ровно верха.
     *
     * Причина в том, когда мы подключаемся. Мьютекс даёт начало кадра в
     * его начале, а мы влезаем в произвольный момент, посреди уже идущей
     * строки. WDMA0 подхватывает поток с середины: верх кадра проходит
     * мимо, низ записывается, и в конце честно приходит «готово».
     *
     * Поэтому первый кадр отдаём на разгон и не считаем его вовсе.
     * Второй начинается с верха, когда мы уже подключены, и приходит
     * целиком.
     */
    for (int frame = 0; frame < 6; frame++) {
        ...
        /*
         * Считаем после каждого кадра, а не только в конце.
         *
         * Пороги очереди дали измеримое: строки заполнились с семи
         * десятых до девяноста шести сотых. Но осталось другое — не
         * хватает верхних пятидесяти шести строк сплошным куском, а это
         * уже не дыры от нехватки полосы.
         *
         * Отличить две причины можно по тому, накапливается ли картинка.
         * Если каждый кадр теряет случайные куски, за шесть кадров буфер
         * заполнится целиком: чего не хватило в одном, придёт в другом.
         * А если верх не пишется никогда — счётчик встанет на месте, и
         * значит дело в самой настройке выреза, а не во времени.
         */
        for (u32 k = 0; k < SNAP_W * SNAP_H; k++)
            if (snap_buf[k] != 0xDEADBEEF) {
                have++;
                if (first < 0)
                    first = (int)(k / SNAP_W);
            }

        kprintf("MDP      : КАДР %d: ЖДАЛ %2u МС, СОСТ %08x, НАБРАНО %5u, С %d\n",
                frame, waited, sta, have, first);
    }
