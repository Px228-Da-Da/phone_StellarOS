...
/*
 * Признаки прерываний оверлея, накопленные между отчётами.
 *
 * Ожидание промежутка между кадрами снимает защёлку — и делает это по
 * шестьдесят раз в секунду, пока оболочка что-нибудь показывает.
 * Поэтому редкий признак вроде переполнения выборки не доживал ни до
 * какого отчёта: его стирали раньше, чем кто-то успевал спросить.
 * Копим здесь, у самого места сброса.
 */
u32 fb_ovl_seen;
...
static int fb_wait_ovl(u32 flag)
{
    ...
    fb_ovl_seen |= mmio_read32(MT_DISP_OVL0_BASE + OVL_INTSTA);
                              ...
                              int px, int py, u32 fg, u32 under)
{
    ...
    for (u32 row = 0; row < g->h; row++) {
        ...
        for (u32 col = 0; col < g->w; col++) {
            ...
            /*
             * Прозрачность самого цвета.
             *
             * Раньше старший байт fg просто выбрасывался: буква всегда
             * ложилась в полную силу, а смешивалось только её сглаженное
             * покрытие. Полупрозрачного текста от этого не получалось
             * вовсе — а он нужен крупным часам, сквозь которые должны
             * просвечивать обои.
             *
             * Умножаем покрытие на прозрачность цвета. Непрозрачный цвет
             * (0xFF...) при этом ведёт себя ровно как прежде, поэтому
             * менять существующие вызовы не пришлось.
             */
            if ((fg >> 24) != 0xFF)
                a = a * (fg >> 24) / 255;
            if (!a)
