...
static void appload_task(void *arg)
{
    ...
    for (;;) {
        ...
        /*
         * Отчёта о шине здесь больше нет.
         *
         * Он стоял в этом цикле, а цикл просыпается двадцать раз в
         * секунду — сто строк в секунду в кольцо консоли на шестнадцать
         * килобайт. Кольцо переполнялось за полторы секунды, и всё
         * остальное — пульс, счётчики, разведка двумерного движка — было
         * вытеснено раньше, чем терминал успевал это прочитать. В логе
         * оставался один отчёт о шине, повторённый сотни раз.
         *
         * Печатает его теперь только пульс, раз в десять секунд, как и
         * задумывалось: показания всё равно постоянные, чаще их снимать
         * незачем.
         */
