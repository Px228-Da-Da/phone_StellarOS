...
/*
 * Гашение экрана в эмуляторе не делаем, но и не молчим об этом.
 *
 * Заглушки понадобились, потому что системный вызов SYS_BLANK один на
 * обе платы, а гасить нечего: слоёв в эмуляторе нет. Без них сборка для
 * эмулятора просто переставала линковаться — что и случилось, и я этого
 * не заметил, потому что собирал только для телефона.
 *
 * ovl_blanked всегда отвечает «не погашен»: соврать здесь значило бы
 * заставить оболочку думать, что экран потушен, и не рисовать вовсе.
 */
void ovl_blank(int on) { (void)on; }
int  ovl_blanked(void) { return 0; }
