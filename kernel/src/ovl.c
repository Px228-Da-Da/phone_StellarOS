 ...
 *
 * Честно о том, чего это НЕ делает: подсветка остаётся включённой.
 * Экран становится чёрным, но светится, и питание она берёт прежнее.
 * Настоящее гашение — это выключить подсветку, а живёт она в отдельной
 * микросхеме MT6370 на шине I2C. В дереве устройства этот чип описан без
 * шины и адреса: у старого драйвера вендора они брались из настроек
 * платы, которых у нас нет. Искать его перебором по шинам — отдельная
 * работа, и делать её наугад посреди этой не стоит.
 *
 * Поэтому здесь ровно то, что можно сделать правильно: экран гаснет,
 * телефон продолжает работать, возврат мгновенный. А выигрыш по питанию
 * появится, когда найдём подсветку.
 */
...
void ovl_blank(int on)
{
    ...
    if (on) {
        ...
        blanked = 1;
        kprintf("ЭКРАН    : ПОГАШЕН (СЛОИ СНЯТЫ, ПОДСВЕТКА ГОРИТ)\n");
    } else {
