...
#define SPI_CMD_FINISH_IE   (1U << 16)
#define SPI_CMD_PAUSE_IE    (1U << 17)
#define SPI_CMD_SAMPLE_SEL  (1U << 6)   /* защёлкивать по другому фронту */
...
/* Когда защёлкивать принятый бит.
 *
 * На высокой частоте микросхема успевает выставить бит не мгновенно, и
 * контроллер читает линию раньше, чем данные на ней устоялись — принятый
 * поток при этом сдвинут ровно на один бит. Лечится либо другим фронтом
 * защёлкивания, либо задержкой в тактах; какое сочетание верное, зависит
 * от разводки платы, поэтому подбираем на живом устройстве. */
u32 spi_lost_status;             /* сколько раз потерян признак завершения */
u32 spi_sample_sel;             /* 0 или 1  */
u32 spi_tick_delay;             /* 0..7     */
...
static void spi_timing(u32 hz)
{
    ...
    /* CFG1 пишем целиком, а не поверх старого. В старших битах живёт
     * задержка защёлкивания принимаемого бита, и оставлять её неизвестной
     * нельзя: именно от неё зависит, на каком такте мы читаем линию.
     * Пауза между посылками — в младшем байте, длина пакета ставится
     * отдельно в spi_transfer. */
    cfg1 = ((cs - 1) & 0xFF) | ((spi_tick_delay & 0x7) << 29);
...
int spi_transfer(const u8 *tx, u8 *rx, u32 len)
{
    ...
    /* Разрешения прерываний ставит и вендорный драйвер. Обработчика у нас
     * нет и не нужно: STATUS0 — регистр состояния прерываний, и без этих
     * битов признак завершения посылки в нём недостоверен. */
    cmd = SPI_CMD_TXMSBF | SPI_CMD_RXMSBF | SPI_CMD_FINISH_IE | SPI_CMD_PAUSE_IE;
    if (spi_sample_sel)
    ...
    /* Ждём завершения.
     *
     * Два способа опробованы и оба сами по себе негодны. Бит ACT — не
     * признак занятости, а спуск: записали единицу, посылка пошла, а
     * читается он нулём всегда. Опрос STATUS0 честнее, но этот регистр
     * сбрасывается чтением, и признак иногда теряется — тогда цикл
     * впустую выкручивает ограничение, посылка считается неудачной, и
     * следующая начинается поверх ещё не досланных байтов.
     *
     * Поэтому ждём по системному счётчику. Срок берём с большим запасом
     * относительно физической длительности посылки: тридцать два байта
     * на шести с половиной мегагерцах — это сорок микросекунд, ждём до
     * трёхсот. Не дождались признака — значит его потеряли, а посылка
     * всё равно давно ушла; считаем её состоявшейся и отмечаем в
     * счётчике, чтобы это не осталось незамеченным.
     */
    {
        u64 freq = read_cntfrq();
        /* Срок считаем от длины посылки: на мегагерце байт идёт восемь
         * микросекунд, берём двадцать пять, плюс сотня на накладные. */
        u64 deadline = read_cntpct() + (freq / 1000000) * (100 + len * 25) + 2;
        int done = 0;

        while (read_cntpct() < deadline) {
            spins++;
            if (mmio_read32(MT_SPI0_BASE + SPI_STATUS0) & 1) {
                done = 1;
                break;
            }
        }
    ...
    /* Очередь приёма вычитываем ВСЕГДА, даже когда принятое не нужно.
     * SPI полнодуплексный: сколько байт ушло, столько и пришло, и если
     * их не забрать, очередь переполнится и следующая посылка не
     * завершится. Именно на этом спотыкалась заливка прошивки: одиночные
     * команды проходили, а на потоке записи шина вставала. */
    for (u32 i = 0; i < len; i += 4) {
