 ...
 *
 * Список MPIDR перебираем, потому что до разбора DTB узнать их неоткуда.
 * Первые восемь — ровно те, что стоят в merlin: сняты с устройства, узлы
 * cpu@000..cpu@003 и cpu@100..cpu@103, то есть два кластера по четыре
 * (см. device-info/dt-tree.txt). Остальные — про запас для плоской
 * нумерации, какая бывает в QEMU и на односкластерных SoC.
 *
 * Несуществующее ядро прошивка отвергнет с INVALID_PARAMETERS — это
 * штатный ответ, а не ошибка, поэтому лишние кандидаты безвредны.
 */
u32 smp_start_secondaries(void)
{
    static const u64 candidates[] = {
        /* merlin: два кластера по четыре ядра — подтверждено на устройстве */
        0x000, 0x001, 0x002, 0x003,
        0x100, 0x101, 0x102, 0x103,
        /* плоская нумерация: QEMU и односкластерные SoC */
