...
static void event_push(u8 id, u8 action, u16 x, u16 y)
{
    ...
    /*
     * Системные полосы обслуживает система, а не приложения.
     *
     * Касание по нижней полоске — «домой»: приложение прячется, и видна
     * оболочка; ещё раз — возвращается. Без этого приложение во всю
     * рабочую область закрывало бы оболочку навсегда.
     *
     * Верхняя полоса пока ничего не делает, но и наружу не уходит:
     * приложение о касаниях по чужой территории знать не должно.
     */
    {
        u32 ax, ay, aw, ah;
