...
static void input_task(void *arg)
{
    ...
    for (;;) {
        ...
        if (now < next) {
            /* До следующего опроса делать нечего. Отдаём процессор:
             * задача не должна крутить пустой цикл, съедая квант. */
            schedule();
            continue;
        }
        next = now + 10;        /* сто опросов в секунду */
        input_scan();
    }
}
...
{
    task_create("ввод", input_task, NULL);
}
