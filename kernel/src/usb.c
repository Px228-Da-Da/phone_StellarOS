...
/*
 * «StellarOS» и «StellarOS CONSOLE».
 *
 * Переименование системы эти строки не задело: они записаны кодами
 * символов, и поиск по слову их не нашёл. Телефон полгода представлялся
 * компьютеру старым именем — мелочь, но именно её видит человек, когда
 * подключает устройство.
 */
static const u8 desc_str1[20] = {
    20, 0x03,
    0x53,0, 0x74,0, 0x65,0, 0x6C,0, 0x6C,0, 0x61,0, 0x72,0, 0x4F,0, 0x53,0
};
static const u8 desc_str2[36] = {
    36, 0x03,
    0x53,0, 0x74,0, 0x65,0, 0x6C,0, 0x6C,0, 0x61,0, 0x72,0, 0x4F,0, 0x53,0,
    0x20,0,
...
/*
 * Печатаем показания повторно, а не один раз.
 *
 * Один раз мы уже пробовали — и не увидели ничего: отчёт уходит при
 * перечислении, то есть в самом начале загрузки, а кольцо консоли
 * шестнадцать килобайт и к моменту подключения терминала давно
 * переполнено. Пока разбираемся с приёмом, отчёт повторяется в пульсе;
 * четыре строки раз в десять секунд — небольшая плата за возможность
 * их прочитать.
 */
 ...
 */
/*
 * Кольцо вывода: сто двадцать восемь килобайт, а было шестнадцать.
 *
 * Шестнадцати не хватало, и это стоило трёх прошивок подряд. Загрузка
 * печатает разом и таблицу разделов, и переговоры с картой, и отчёты
 * проверок в EL0 — за пару секунд куда больше, чем провод успевает
 * отдать. Кольцо оборачивалось, и вместе с лишним уносило нужное:
 * сначала отчёт о шине, потом разведку движка, потом целиком TICK с 10
 * по 40. Каждый раз это выглядело как «железо молчит», а молчала
 * консоль.
 *
 * Память здесь не жалко: свободных страниц под сто двадцать тысяч, а
 * сто двадцать восемь килобайт — это две с половиной секунды запаса при
 * нынешней скорости отдачи. Загрузочного вывода столько не бывает.
 */
#define CON_RING 131072                 /* степень двойки: маска вместо деления */
...
static void ring_drain_locked(void)
{
    ...
    /*
     * За один раз отдаём не больше шестнадцати пакетов.
     *
     * Отдача идёт под замком и с закрытыми прерываниями, поэтому
     * вычерпывать кольцо целиком нельзя: это удержало бы все ядра на
     * время двух тысяч посылок. Отдача вызывается по тику, сто раз в
     * секунду, так что шестнадцать пакетов — это сто килобайт в секунду.
     *
     * Было восемь, и утверждалось, что «это больше, чем мы печатаем».
     * Утверждение оказалось неверным: на загрузке печатается заметно
     * больше, кольцо переполнялось и уносило нужное. Удвоение здесь и
     * восьмикратное кольцо — две стороны одной починки.
     */
    for (u32 packet = 0; packet < 16 && ring_tail != ring_head; packet++) {
        u32 n = 0;
