...
/* Настройки доступа слоя к памяти: пороги и размер очереди выборки.
 * Загрузчик задаёт их только тем слоям, которыми пользуется сам; у
 * остальных там нули — очередь нулевого размера, читать нечем. Слой при
 * этом выглядит полностью настроенным и включённым. */
#define OVL_RDMA0_GMC       0x00C8          /* пороги, шаг 0x20 */
#define OVL_RDMA0_SLOW      0x00CC
#define OVL_RDMA0_FIFO      0x00D0          /* размер очереди, шаг 0x20 */
#define OVL_RDMA0_GMC_S2    0x01E0          /* шаг 4, а не 0x20 */
...
/* --- OVL: тот блок, который читает наш буфер из памяти ---
 * Раскладка из ddp_reg_ovl.h вендорного ядра. Важно различать: RDMA
 * отдаёт кадр панели, а из памяти читает именно OVL, поэтому свободен
 * наш буфер не тогда, когда кадр ушёл на экран, а когда OVL дочитал.
 */
#define OVL_STA             0x0000
#define OVL_STA_RUN         (1U << 0)       /* блок сейчас читает кадр */
#define OVL_INTEN           0x0004
#define OVL_INTSTA          0x0008
#define OVL_INT_REG_CMT     (1U << 0)       /* настройки вступили в силу */
#define OVL_INT_FRAME_CPL   (1U << 1)       /* кадр дочитан из памяти  */
#define OVL_INT_START       (1U << 14)      /* начало чтения кадра     */
