...
void kmain(u64 dtb_phys)
{
    ...
    /* Художник рисует в своём окне и просит показать его на экране.
     * Это первая программа, которая что-то видно делает. */
    uspace_spawn_image(IMG_PAINT);

    /* Следопыт двигает своё окно за пальцем — и ждёт касания сном,
     * а не опросом. */
