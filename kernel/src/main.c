...
static void fb_flip_demo(void)
{
    ...
    /* 60 кадров в секунду: период в тиках системного счётчика */
    period = read_cntfrq() / 60;
    t_start = read_cntpct();
    ...
    for (u32 i = 0; i < frames; i++) {
        ...
        /* Держим постоянный темп.
         *
         * Без этого кадр показывается сразу, как нарисован, а рисуются они
         * за разное время — движение выходит рваным независимо от того,
         * быстро мы рисуем или медленно. Настоящее решение — ждать обратного
         * хода луча, но прерывания от контроллера дисплея у нас пока нет,
         * поэтому равняемся по системному счётчику: он тикает независимо
         * от нас и врать не станет. */
        while (read_cntpct() < t_start + (u64)(i + 1) * period)
            ;
    }
