 ...
 *
 * Смысл проверки именно в этом. В прошлой демонстрации полоса двигалась
 * потому, что мы каждый кадр перекрашивали пиксели — 836 микросекунд на
 * кадр. Здесь пиксели рисуются один раз, при подготовке, а дальше каждый
 * кадр меняется одно число: положение слоя. Если движение видно, а время
 * отрисовки нулевое — значит складывает картинку железо.
 */
...
static void ovl_demo(void)
{
     ...
     *
     * Берём память, пригодную для внешнего блока: целыми блоками по два
     * мегабайта. Обычное выделение здесь однажды утащило за собой кучу и
     * кольцо вывода — они лежали в том же блоке и стали некэшируемыми. */
    ovl_panel = pmm_alloc_dma(PW * PH * 4);
    pages = (u32)((2UL * 1024 * 1024) / PAGE_SIZE);
    kprintf("OVL: ПАНЕЛЬ %u СТРАНИЦ -> %p\n", pages, (void *)ovl_panel);
    if (!ovl_panel) {
        kprintf("OVL: НЕТ ПАМЯТИ ПОД ПАНЕЛЬ\n");
        return;
    }
    mmu_set_range_nc((u64)(uintptr_t)ovl_panel, (u64)pages * PAGE_SIZE);
    ...
    /* Фон под панелью — основной кадр, он у нас уже есть */
    fb_clear(0xFF101840);

    /* Слой 2: сплошной цвет во весь экран, наполовину прозрачный.
     * Буфера у него нет вовсе — цвет лежит в регистре, читать нечего. */
    ovl_layer_color(2, 0x60000000, 0, 0, w, h);

    /* Слой 1: наша панель поверх затемнения */
    {
        int r = ovl_layer_set(1, ovl_panel, 0, 0, PW, PH, PW * 4, 255);

        if (r) {
            kprintf("OVL: СЛОЙ 1 НЕ ВСТАЛ, КОД %d\n", r);
            return;
        }
    }
    ovl_dump();

    kprintf("OVL: ПАНЕЛЬ %ux%u ПОШЛА, ПИКСЕЛИ БОЛЬШЕ НЕ ТРОГАЕМ\n", PW, PH);

    for (u32 i = 0; i < frames; i++) {
        /* Полный ход туда и обратно за все кадры */
        u32 half = frames / 2;
        ...
        /* Заодно плавно меняем прозрачность: тоже одна запись */
