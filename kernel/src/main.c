...
static void ovl_demo(void)
{
    ...
    /*
     * Загрузчик оставил слой 3 включённым во весь экран и непрозрачным,
     * а он выше наших по порядку наложения. Убираем его на три секунды:
     * если панель появится только сейчас — причина в нём.
     */
    kprintf("OVL: ЭТАП 3 — ТО ЖЕ БЕЗ СЛОЯ ЗАГРУЗЧИКА, 3 СЕК\n");
    ovl_layer_off(3);
    delay_ms(3000);
     ...
     */
    kprintf("OVL: ЭТАП 4 — БЕЗ ОСНОВНОГО КАДРА, 3 СЕК\n");
    ovl_base_layer(0);
    delay_ms(3000);
    ovl_base_layer(1);

    kprintf("OVL: ЭТАП 5 — ДВИЖЕНИЕ\n");
