...
#define BL_MAX          2047
/*
 * Нижний предел яркости.
 *
 * Было 24 из 2047 — около процента. На этой матрице это неотличимо от
 * выключенного экрана, и телефон после такого не поднять: кнопка питания
 * яркость не меняет, а чтобы поднять ползунок, его надо увидеть. Ловушка
 * получилась ровно та, от которой я оберегался в backlight_level, — и
 * оберёгся недостаточно.
 *
 * Десятая часть шкалы: тускло, но видно всегда.
 */
#define BL_MIN          205
 ...
 *
 * Сначала стояла половина — с неё нагрев и пропал. Три четверти выбраны
 * человеком, который держит этот телефон в руках, и это единственный
 * верный способ выбрать яркость: числом её не выведешь.
 */
#define BL_DEFAULT      75
...
int backlight_level(u32 percent)
{
    ...
    bl_percent = percent;
    kprintf("ПОДСВЕТКА: ЯРКОСТЬ %u%% (УРОВЕНЬ %u ИЗ %u)\n",
            percent, level, (u32)BL_MAX);
    return 0;
}
...
void backlight_set(int on)
{
    ...
    /*
     * Записанное проверяем чтением обратно.
     *
     * Сначала здесь стояла одна запись, и её ответ выбрасывался. Это
     * ровно тот путь, по которому телефон однажды не вернулся с
     * погашенного экрана: если бы запись не прошла, подсветка осталась
     * бы выключенной, а система считала бы экран возвращённым — и в
     * журнале не было бы ни слова. Молчащая неудача на единственной
     * дороге назад недопустима.
     */
    if (bl_write(LM_ENABLE, want)) {
        kprintf("ПОДСВЕТКА: ЗАПИСЬ %02x НЕ ПРОШЛА\n", want);
        return;
    }
    if (bl_read(LM_ENABLE, &got) || got != want)
        kprintf("ПОДСВЕТКА: ЗАПИСАЛ %02x, В МИКРОСХЕМЕ %02x\n", want, got);
}
