...
void splash_task(void *arg)
{
     ...
     *
     * Выключение двойной буферизации возвращает показ на ПЕРВЫЙ буфер, а
     * в нём лежит кадр на одну смену старше — с логотипом. Панель
     * успевала его показать, следом по нему шла заливка, и только потом
     * появлялись полосы: ровно это и выглядело как моргание.
     *
     * Поэтому конечный вид рисуется в оба буфера, и переключение
     * перестаёт что-либо менять. Два лишних прохода по кадру стоят
     * тридцать миллисекунд один раз за загрузку — дешевле, чем моргок,
     * который видит каждый.
     */
    if (buffered) {
        paint_all();
        ui_status_draw();
