...
void splash_show(void)
{
    ...
    /*
     * И сразу полосы — в тот же самый первый кадр.
     *
     * Панель обязана существовать с того мгновения, как существует
     * экран. Всё, что делается позже, — задача плавности, оболочка,
     * подмена буферов — только сохраняет её; появиться ей больше негде,
     * а значит и промежутку, когда её нет, взяться неоткуда.
     */
    ui_status_repaint();
}
...
void splash_task(void *arg)
{
    ...
    /*
     * Заливать кадр здесь незачем, и это была дорогая ошибка.
     *
     * Включение двойной буферизации само копирует показываемый буфер во
     * второй — значит фон в обоих уже одинаков, вместе с логотипом и
     * полосами из первого кадра. Две заливки поверх этого стирали
     * логотип и стоили на телефоне полторы сотни миллисекунд записей
     * мимо кэша: задача до своего цикла доходила только через четверть
     * секунды. За это время оболочка успевала занять слой и показаться —
     * с пустым верхом, потому что полосы рисуются в цикле.
     *
     * Отсюда и «мигание после загрузки», которое не поддавалось ни
     * одной правке внутри цикла: цикл к тому мгновению ещё не начался.
     */
    ...
    while (!done) {
        ...
        /*
         * Полосы кладём сами, с первого же кадра заставки.
         *
         * Оболочка занимает свой слой и становится видна задолго до
         * того, как заставка уходит, — а в полосах у заставки лежит один
         * голый градиент. Человек видит рабочий стол с пустым верхом, и
         * лишь потом туда въезжают часы: это и есть мигание панели.
         *
         * Сначала я привязал их к этапу «программы пошли» — и промахнулся
         * на четверть секунды: оболочка запускается заметно раньше, чем
         * ядро доходит до этой отметки. Видео показало ровно семь кадров
         * пустой панели, тридцать в секунду. Поэтому никаких этапов:
         * полосы есть всегда, сколько есть кадр.
         *
         ...
         */
        /*
         * Сторожа включаем здесь, а не после ухода заставки: прошлый раз
         * он начинал следить, когда всё уже случилось.
         */
        fb_watch_strips(1);
        ui_status_repaint();
