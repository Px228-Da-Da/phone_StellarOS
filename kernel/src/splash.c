...
/* Экран принадлежит заставке, пока она не убрана до конца */
...
void splash_task(void *arg)
{
    ...
    while (!done) {
        ...
        /* Сторож полос смотрит и на переход тоже */
        fb_watch_strips(1);
     ...
     * Смена караула.
     *
     * Полосы кладём в оба буфера, а рабочую область пока НЕ трогаем: в
     * ней ещё логотип, и он обязан дожить до появления стола. Уберём
     * его раньше — человек увидит несколько кадров пустого фона между
     * уходом заставки и выходом окон на слои, а это ровно то самое, что
     * читается как мигание.
     *
     * Порядок здесь и есть всё содержание правки: сначала панель,
     * потом стол, и только потом уборка того, что уже накрыто.
     */
    if (buffered) {
        ui_status_repaint();
        fb_flip();
        ui_status_repaint();        /* и во второй буфер тоже */
        fb_double_buffer(0);
    } else {
        ui_status_repaint();
    }
    ...
    /*
     * Экран сдан. Окна, которые всё это время ждали, выходят на слои —
     * рабочий стол появляется целиком и сразу, вместе с полосами, а не
     * посреди загрузки с пустым верхом.
     */
    gone = 1;
    window_reveal_all();

    /*
     * И только теперь убираем логотип с ползунком: они уже накрыты
     * окнами, и никто не увидит, как они исчезают.
     */
    paint_work();
