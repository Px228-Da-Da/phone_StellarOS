...
while ($true) {
    ...
    $p.ReadTimeout = 500
    # DTR — это сигнал ядру "порт открыт, можно отдавать накопленное".
    # По умолчанию .NET его не поднимает, и ядро молчит: оно копит вывод
    # в кольце, пока никто не читает, чтобы не потерять начало загрузки.
    $p.DtrEnable = $true

    try { $p.Open(); $p.DtrEnable = $true }
    catch {
